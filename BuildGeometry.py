...
class BuildGeometry:
    ...
    def __computeGantryInfo(self,g):
        ...
        self.image.matrixSize = [self.sinogram.nRadialBins,self.sinogram.nRadialBins,2*self.scanner.nCrystalRings-1] #172,172,127
    ...
    def buildMichelogram(self):
        a = np.transpose(np.arange(1,self.scanner.nCrystalRings**2+1).reshape(self.scanner.nCrystalRings,self.scanner.nCrystalRings)) #64*64 range from 1 to 4096
        b = np.arange(-1*self.scanner.maxRingDiff,self.scanner.maxRingDiff + 1).reshape(self.sinogram.nSegments,self.sinogram.span)   #11*11 range from -60 to 60
        direction = self.sinogram.nSegments//2 #5
        isodd = np.remainder(b[direction,0],2) #1 
        Segments = []
        maxNumberOfPlanesPerSeg = np.zeros([self.sinogram.nSegments, 2],dtype='int16') # 11*2
        ...
        self.sinogram.numberOfPlanesPerSeg = np.sum(maxNumberOfPlanesPerSeg,axis=1) # 27,49,71,93,115,127,115,93,71,49,27
        self.sinogram.totalNumberOfSinogramPlanes = np.sum(self.sinogram.numberOfPlanesPerSeg) #837
        return Segments
    ...
    def LorsAxialCoor(self):
        ...
        Segments = self.buildMichelogram()
        z_axis = self.scanner.zCrystalDimCm* np.arange(-(self.scanner.nCrystalRings -1)/2,(self.scanner.nCrystalRings -1)/2+1) #(64,:)
        axialCoorPerSeg = []
    ...
    def LorsTransaxialCoor(self):
        ...
        halfNumberOfRadialBins = (self.sinogram.nRadialBins)//2+1 # Before interleaving 87
        R = np.empty((self.scanner.nCrystalsPerRing,3), dtype=object)
        V = np.zeros([halfNumberOfRadialBins,2],dtype='int16') #(87,2)
        ...
        for i in range(self.scanner.nCrystalsPerRing//2):
            ...
            P1=R[idx,0]
            P2=R[idx,1]
            xy1[i,0:self.sinogram.nRadialBins:2,:] = P1[0:-1,:] #(252,172,2)
            xy1[i,1:self.sinogram.nRadialBins:2,:] = (P1[0:-1,:]+P1[1::,:])/2
            xy2[i,0:self.sinogram.nRadialBins:2,:] = P2[0:-1,:] #(252,172,2)
    ...
    def buildSystemMatrixUsingSymmetries(self,save_dir=None, reconFovRadious=None, is3d = False,ncores = 1):
        ...
        if ncores==1:
            for i in range(N):
                ...
                np.save(save_dir+'geoMatrix-'+str(i)+'.npy', geoMatrix) #126*172
                if self.scanner.isTof:
    ...
    def loadSystemMatrix(self,save_dir,is3d = False, tof=True,reconFovRadious=None):
        ...
        save_dir = save_dir + self.engine.bar
        tic = time.time()
        if (not is3d and not os.path.isfile(save_dir+'geoMatrix-0.npy')) or (is3d and not os.path.isfile(save_dir+'geoMatrix-1.npy')): #路径中有没有.npy文件的系统矩阵，没有的话生成系统矩阵
    ...
    def iSensImageBatch2D(self, prompts,AN=None, nsubs = 1, psf=0):
        ...
        return iSensImageSubBatch,sensImageSubBatch
    ...
    def OSEM2D(self, prompts,img=None,RS=None, AN=None, iSensImg = None, niter=100, nsubs=1,  tof=False, psf=0):
        ...
        if AN is None:
            ...
            if np.ndim(AN)==2:
                 AN = AN[None,:,:]
        if iSensImg is None:
            iSensImg,SensImg = self.iSensImageBatch2D(AN, nsubs, psf)
        if np.ndim(iSensImg)==2:
            iSensImg = iSensImg[None,:,:]
                 
        # matrixSize = [128,128,172] #for real data
        matrixSize = self.image.matrixSize
        q = self.sinogram.nAngularBins//2
    ...
    def forwardDivideBackwardBatch2D(self, imgb, prompts, RS=None, AN=None, nsubs=1, subset_i=0, tof=False, psf=0):
        ...
        if img.shape[0]!=batch_size:
            raise ValueError("1st img dimension dosn't match batch_size")
        nVoxls = np.prod(self.image.matrixSize[:2]) #29584=172*172
        img = np.reshape(img,[batch_size,nVoxls],order='F') 
