...
class RSTR(nn.Module):
    """Residual Swin Transformer based regularizer (RSTR).
    ...
    def __init__(self,input_dim, dim, input_resolution, depth, num_heads, window_size,
        ...
        super(RSTR, self).__init__()
        self.conv_first = nn.Sequential(
            nn.Conv2d(input_dim, dim, 3, 1, 1),
            nn.BatchNorm2d(dim)
        )

        self.conv_last = nn.Conv2d(dim, input_dim, 3, 1, 1)
        self.dim = dim
        ...
        self.patch_embed = PatchEmbed(
            img_size=self.img_size, patch_size=patch_size, in_chans=0, embed_dim=dim,
            norm_layer=None)
